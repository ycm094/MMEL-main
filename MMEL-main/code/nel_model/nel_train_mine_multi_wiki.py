...
from nel_dataset_multi_opt import NELDataset
from nel3 import NELModel
from metric_topk import cal_top_k_mine, cal_top_k_mine_multi, cal_top_k_mine_multi2
...
def parse_arg():
    ...
    parser.add_argument(
        "--dir_prepro",
        default=root_path+"wiki_data/prepro_data/nel",
        type=str,
        # required=True,
    ...
    parser.add_argument(
        "--path_ans_list",
        default=root_path+"wiki_data/prepro_data/dm/qids_ordered.json",
        type=str,
        help="Path to ordered answer list.",
    )
    parser.add_argument(
        "--path_candidates",
        default=root_path+"wiki_data/prepro_data/nel/search_top100.json",
        type=str,
        help="Path to search results.",
    )
    parser.add_argument(
        "--path_neg_config",
        default=root_path+"wiki_data/prepro_data/dm/neg_cache/neg.json",
        type=str,
        help="Path to neg.json",
    )
    parser.add_argument(
        "--dir_img_feat",
        default=None, # "data/prepro_data/nel",
        type=str,
    ...
    parser.add_argument(
        "--dir_output",
        default="/data0/ycm/CODE/MMEL/output_data_mine_multi/nel/wiki_img_new100_nel3_util_clip_prompt",
        type=str,
        # required=True,
    ...
    parser.add_argument(
        "--config_name", default="bert-base-cased", type=str, help="Pretrained config name or path if not the same as model_name"
    )
    parser.add_argument(
        "--tokenizer_name",
        default="",
    ...
    parser.add_argument(
        "--num_attn_layers", default=2, type=int, help="Num of attention layers"
    )
    parser.add_argument(
        "--output_size", default=768, type=int, help="Output size."
    )
    parser.add_argument(
        "--feat_cate", default='w', help="word (w), phrase(p) and sentence(s), default: wp"
    )
    parser.add_argument(
    ...
    # loss
    parser.add_argument(
        "--loss_function", default="None", type=str, help="Loss: triplet | circle | None"
    )

    parser.add_argument(
        "--loss_margin", default=0.3, type=float, help="margin of circle loss."
    )
    parser.add_argument(
    ...
    # circle loss
    parser.add_argument(
        "--loss_scale", default=16, type=int, help="Scale of circle loss."
    )
    ...
    parser.add_argument("--multi_gpus", default=False, action="store_true", help="Using multiple CUDAs when available")
    ...
    parser.add_argument("--gpu_id", type=int, default=3, help="id of gpu")
...
def train(args, train_dataset, model, nel_model, answer_list, tokenizer, fold=""):
    ...
    for epoch in range(epochs_trained, int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            B, N, L = batch["all_pos_input_ids"].size()
            bert_inputs = {
                "input_ids": batch["all_pos_input_ids"].view(B*N, L),
                "attention_mask": batch["all_pos_input_mask"].view(B*N, L)}
            pos_bert_out = model(**bert_inputs)

            bert_inputs = {
                "input_ids": batch["all_neg_input_ids"].view(B*N, L),
                "attention_mask": batch["all_neg_input_mask"].view(B*N, L)}
            neg_bert_out = model(**bert_inputs)
            nel_inputs = {"bert_feat": (pos_bert_out[0].view(B, N, L, -1), neg_bert_out[0].view(B, N, L, -1)),
                          "img": batch["img_feat"],
            ...
            outputs = nel_model(**nel_inputs, mode='train', multi=True)
            loss, triplet_loss = outputs[0]
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if step_start_time is None:
                        ...
                        logger.info(f"loss_{global_step}: {(tr_loss - logging_loss) / args.logging_steps}, {loss:.4f}, {triplet_loss:.4f}, epoch {epoch+1}: {step + 1}/{num_steps}")
                    else:
                        log_tim = (time() - step_start_time)
                        logger.info(
                            f"loss_{global_step}: {(tr_loss - logging_loss) / args.logging_steps}, {loss:.4f}, {triplet_loss:.4f}, epoch {epoch+1}: {step + 1}/{num_steps} ({log_tim:.2f}s/50step)")
                        step_start_time = time()
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    # Log metrics
                    if (
                            args.local_rank == -1 and args.evaluate_during_training # and global_step > 2000
                    ):
                        logger.info(f"********Global step {global_step}, DEV Evaluation********", )
                        results, _ = evaluate(args, model, nel_model, answer_list, tokenizer, mode="test")[:2]
...
def evaluate(args, model, nel_model, answer_list, tokenizer, mode, prefix=""):
    ...
    with torch.no_grad():
        eval_dataloader.dataset.set_max_num(1)
        # eval_dataloader.dataset.set_max_num(5)
        for i, batch in enumerate(eval_dataloader):
            if i % 20 == 0:
                print(i) # 3775 / batch_size
            # if i == 10:
            #     break
            if i == int(3775 / 8) - 1:  # 1459 for nytimes and 3775 for wiki
                eval_dataloader.dataset.set_max_num(5)
            ...
            B, CN, L = batch["all_pos_input_ids"].size()
            bert_inputs = {
                "input_ids": batch["all_pos_input_ids"].view(B*CN, L).long(),
                "attention_mask": batch["all_pos_input_mask"].view(B*CN, L).long()}
            pos_bert_out = model(**bert_inputs)
            B, CN, N, L = batch["search_res_tokens"].size()
            bert_inputs = {
                "input_ids": batch["search_res_tokens"].view(B*CN*N, L).long(),
                "attention_mask": batch["search_res_mask"].view(B*CN*N, L).long()}
            neg_bert_out = model(**bert_inputs)
            nel_inputs = {"bert_feat": (pos_bert_out[0].view(B, CN, L, -1), neg_bert_out[0].view(B, CN, N, L, -1)),
                          "img": batch["img_feat"],
                          "bert_mask": batch["input_mask"],
                          "pos_feats": batch["pos_sample"],
                          "neg_feats": batch["neg_sample"],
            ...
            outputs = nel_model(**nel_inputs, mode='eval', multi=True)
            tmp_eval_loss, query, feats = outputs[:3]
            ...
            rank_list, sim_p, sim_n = cal_top_k_mine_multi2(args, batch['count'], batch['ent_num'], query[0].view(B, CN, -1), query[1].view(B, CN, N, -1), feats)
